...
class HLoss(Loss):
            ...
            for i in range(pred.shape[0]):
                ...
                for j in range(tpred.shape[0]):
                    ...
                    if t[-1] == p[-1]:
                        ...
                        # else:
                            # print(p,t)
            if len(keepnums) == pred.shape[0]:
                res.append(pred)
            else:
                if len(keepnums) == 0:
                    res.append(torch.tensor([]).to(pred.device))
                else:
                    res.append(torch.cat([pred[i].unsqueeze(0) for i in keepnums]).to(pred.device))
        return res
    ...
    def dis_loss(self, preds, batch, tpreds, tfpreds):
        ...
        #             mask[x:x+w, y:y+h] = 0
            ...
            loss[3] += torch.nn.functional.mse_loss(sm, tm.detach())#((tm[b] -sm[b])**2*mask.unsqueeze(0)).mean()
        for p,tp in zip(preds[0], tpreds[0][1]):
            # import pdb; pdb.set_trace()
            loss[3] += torch.nn.functional.mse_loss(p, tp.detach())
        loss[0] *= self.hyp.box  # box gain
        loss[1] *= self.hyp.cls  # cls gain
        loss[2] *= self.hyp.dfl  # dfl gain
        loss[3] *= self.hyp.dis
...
if __name__ == '__main__':
    train(cfg='cfg/dota_default_inc.yaml')
