...
class DetectionTrainer(BaseTrainer):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    self.loss, self.loss_items = self.criterion(preds, batch, tpreds)
                    if rank != -1:
    ...
    def criterion(self, preds, batch, tpreds=None):
        if not hasattr(self, 'compute_loss'):
            # TODO dis loss
            self.compute_loss = HLoss(de_parallel(self.smodel))
        return self.compute_loss(preds, batch, tpreds)
...
class Loss:
    ...
    def __call__(self, preds, batch, tpreds):
        if tpreds is None:
            return self.nodisloss(preds, batch)
        else:
            return self.dis_loss(preds, batch, tpreds)
    ...
    def dis_loss(self, preds, batch, tpreds):
...
class HLoss(Loss):
    ...
    def dis_loss(self, preds, batch, tpreds):
        ...
        allbatch = self.postprocess(tpreds[0], batch, imgsz)
...
if __name__ == '__main__':
    train(cfg='ultralytics/yolo/cfg/default_inch.yaml')
