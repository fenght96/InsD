...
class DistillationBaseModel(BaseModel):
    # YOLOv8 distillation model
    ...
    def _forward_augment(self, x):
        ...
        for si, fi in zip(s, f):
            xi = scale_img(x.flip(fi) if fi else x, si, gs=int(self.stride.max()))
            yi, feat = self._forward_once(xi)  # forward
            # cv2.imwrite(f'img_{si}.jpg', 255 * xi[0].cpu().numpy().transpose((1, 2, 0))[:, :, ::-1])  # save
            yi = self._descale_pred(yi, fi, si, img_size)
            y.append(yi)
            feats.append(feat)
        y = self._clip_augmented(y)  # clip augmented tails
        return torch.cat(y, -1), feats  # augmented inference, train
...
class DistillationModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):
        super().__init__()
        self.old_model = DistillationBaseModel(cfg, ch, nc, verbose)
        self.new_model = DistillationBaseModel(cfg, ch, nc, verbose)

        self.names = {i: f'{i}' for i in range(self.yaml['nc'])}  # default names dict
        self.inplace = self.yaml.get('inplace', True)

        # Build strides
        m = self.old_model[-1]  # Detect()
        if isinstance(m, (Detect, Segment)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
    ...
    def forward_old(self, x, augment=False, profile=False, visualize=False):
        with torch.no_grad():
            y = self.old_model(x, augment=False, profile=False, visualize=False)[-1]
            return y
    def forward_new(self, x, augment=False, profile=False, visualize=False):
        y, feats = self.new_model(x, augment=False, profile=False, visualize=False)
        return y, feats

    def forward(self, x, augment=False, profile=False, visualize=False):
        new_y, new_feats = self.forward_new(x, augment, profile, visualize)
        if self.train:
            old_feats = self.forward_old(x, augment, profile, visualize)
            return new_y, new_feats, old_feats
        return new_y, None
